...
while (!won) {
  // Will loop over block until condition isn't true
  history.count += 1;
...
/*find max value of lowwer bound in the searching history*/
function findMaxLow(history) {
  let max = Number.MIN_VALUE;
  history.results.forEach(result => {
    max = Math.max(max, result.low);
  });
  return max;
}

/*find min value of upper bound in the searching history*/
function findMinHigh(history) {
  let min = Number.MAX_VALUE;
  history.results.forEach(result => {
    min = Math.min(min, result.high);
  });
  return min;
}

/*count same pos record in results.similar*/
...
function thinkAbout(wordInfo) {
  ...
  // EDIT BELOW THIS
  /*sort the origin list try to use binary-searching*/
  const array = wordInfo.allWords.concat().sort();
  return array;
  // EDIT ABOVE THIS
}
...
function pickGuess(wordInfo, history) {
  ...
  // EDIT BELOW THIS
  /*use binary search to do pick the next guess number*/
  if (history.results.length == 0) {
    /*save the last time guess Index*/
    history.guessIdx = 0;
    return wordInfo.magic[0];
  }

  const low = findMaxLow(history);
  const high = findMinHigh(history);

  const mid = low + Math.floor((high - low) / 2);

  /*save the last time guess Index*/
  history.guessIdx = mid;

  return wordInfo.magic[mid];
...
function compareLetters(guess, wordInfo) {
  ...
  const idxGuess = history.guessIdx;

  result.low = guess < base ? idxGuess + 1 : 0;

  result.high = guess > base ? idxGuess - 1 : wordInfo.allWords.length - 1;

  result.similar = countSameLetter(base, guess);
  result.won = guess === base ? true : false;
