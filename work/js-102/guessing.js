...
/*Check if searched in history*/
function isSearched(result, word) {
  for (let w of result) {
...
function thinkAbout(wordInfo) {
  ...
  // EDIT BELOW THIS
  /*Have submitted a more optimized solution in extra part,
    This is a dummb solution, so no code here*/
  // EDIT ABOVE THIS
}
...
function pickGuess(wordInfo, history) {
  ...
  // EDIT BELOW THIS
  let i = 0;
  while (i < wordInfo.allWords.length) {
    if (isSearched(history.results, wordInfo.allWords[i])) {
      i++;
    } else {
      break;
    }
  }
  return wordInfo.allWords[i];
  // EDIT ABOVE THIS
}
